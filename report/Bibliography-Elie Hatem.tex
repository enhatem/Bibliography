...
\footnotetext{\url{https://newatlas.com/drones/muscle-signals-drone-control/\#gallery:2}}
...
\setlength{\itemindent}{-.5in}
	\item [] \textbf{Chapter 2} is devoted to the dynamics of quadrotors. Specifically, the dynamic model of the quadrotor will be presented by using the Newton-Euler formalism. In addition, the state-space model of the quadrotor will also be presented.

	\item [] \textbf{Chapter 3} provides an overview of the different potential control methods that can be used during the master thesis in order to properly control the quadrotor. 

	\item [] \textbf{Chapter 4} provides detailed explanations of how multi-flip maneuvers can be handled and the link between a quadrotor performing a flip and a parallel robot crossing a singularity will be given. Then, a literature review is included in order to show how the problem is tackled by different researches.

	\item [] \textbf{Chapter 5} is devoted to trajectory optimization. By using trajectory optimization, it will be possible to create feasible trajectories for quadrotors to perform the maneuvers in constrained environments.
	
	\item [] \textbf{Chapter 6} represents the conclusion section. It explains the contributions in quadrotor control and singularity crossing which are expected during this research work.

	\item [] \textbf{Bibliography} lists the different references cited in the chapters above.
